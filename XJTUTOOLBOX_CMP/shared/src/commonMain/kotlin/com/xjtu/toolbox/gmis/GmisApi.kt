...
data class GmisScoreItem(
    val courseName: String, val coursePoint: Double, val score: Double,
    val type: String, val examDate: String, val gpa: Double
)
...
class GmisApi(private val login: GmisLogin) {
    ...
    private fun parseCurrentSemester(html: String): String? {
        return Ksoup.parse(html).select("select#drpxq option[selected]").first()?.text()?.trim()
    }
    ...
    private fun parseScheduleHtml(html: String): List<GmisScheduleItem> {
        ...
        for (match in pattern.findAll(html.replace("\n", " "))) {
    ...
    private suspend fun fetchSchedulePage(): String {
        val useCache = schedulePageCache != null && currentTimeMillis() - lastModified < 600_000
        return if (useCache) {
            schedulePageCache!!
        } else {
            val data = login.client.get("https://gmis.xjtu.edu.cn/pyxx/pygl/xskbcx").bodyAsText()
    ...
    suspend fun getSchedule(timestamp: String? = null): List<GmisScheduleItem> {
        val data: String
        if (timestamp == null) {
            ...
            data = login.client.get("https://gmis.xjtu.edu.cn/pyxx/pygl/xskbcx/index/$termValue").bodyAsText()
        }
        if (termValueMap == null) termValueMap = parseSemesterOptions(data)
        return parseScheduleHtml(data)
    }

    suspend fun getScore(): List<GmisScoreItem> {
        val html = login.client.get("https://gmis.xjtu.edu.cn/pyxx/pygl/xscjcx/index").bodyAsText()
