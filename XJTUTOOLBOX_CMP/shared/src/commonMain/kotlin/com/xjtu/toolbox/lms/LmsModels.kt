...
data class LmsUpload(
    ...
    val readableSize: String
        get() = when {
            ...
            else -> "${(size / (1024.0 * 1024.0)).let { v -> val r = (v * 10).toLong(); "${r / 10}.${r % 10}" }} MB"
        }
}
...
data class NsaStudentProfile(
    ...
    companion object {
        ...
        fun fromJson(json: String): NsaStudentProfile? = try {
            val obj = json.safeParseJsonObject()
            val detailsList = mutableListOf<Pair<String, String>>()
            obj["details"]?.let { el ->
                val arr = el as? kotlinx.serialization.json.JsonArray ?: return@let
                arr.forEach { item ->
                    val p = item.jsonObject
                    detailsList.add(
                        p["k"]!!.jsonPrimitive.content to p["v"]!!.jsonPrimitive.content
                    )
                }
            }
            NsaStudentProfile(
                name = obj["name"]?.jsonPrimitive?.content ?: "",
                studentId = obj["studentId"]?.jsonPrimitive?.content ?: "",
                college = obj["college"]?.jsonPrimitive?.content ?: "",
                major = obj["major"]?.jsonPrimitive?.content ?: "",
                details = detailsList
            )
        } catch (e: Exception) {
            Logger.w("NsaApi", "fromJson failed: ${e.message}"); null
        }
    }
}
