...
object AppChangelog {
    ...
    val ENTRIES: List<Pair<String, VersionChangelog>> = listOf(
        ...
        "4.5" to VersionChangelog(
            items = listOf(
                "🎓" to "「我的」页新增学籍档案：证件照、书院、专业班级，以及辅导员和班主任的联系方式",
                "📊" to "首页显示实时状态：下节课、校园卡余额、本周出勤率、待评教、体测成绩、最新作业等",
                "🔔" to "屁岱会主动提醒余额不足、新成绩、快上课了，可在屁岱设置里关闭",
                "📥" to "仲英学辅资料站支持下载；「下载记录」改为「下载管理」，支持多选批量删除",
                "🪟" to "修复添加账号、编辑日程、新版本提示等弹窗点了没反应的问题",
                "🏟️" to "场馆预订改为手动滑动验证，不再因自动识别出错导致预约失败",
                "📱" to "移动交大修复校外无法进入，并支持网页内使用相机",
                "🔍" to "修复屁岱联网搜索搜不到结果的问题",
                "📢" to "通知公告恢复正常查询",
                "📅" to "修复短学期时课程表缺少上一学期的问题",
                "🎫" to "加餐券改为滚动到底自动加载"
            )
        ),
        ...
        "4.1" to VersionChangelog(
            items = listOf(
                ...
                "⏰" to "屁岱可调用安卓标准闹钟和系统日历，帮你设置提醒与日程",
                "🛠️" to "新增登录诊断工具，屁岱可读取脱敏登录状态和近期认证事件，辅助排查功能不可用",
                "🔎" to "联网搜索与网页抓取增强，结果保留链接、最终 URL 和页面内链接，方便连续阅读",
                "🏫" to "优化空闲教室筛选弹窗，并教会屁岱结合刚解放教室、近期课程位置、图书馆 24h 区与楼栋方位推荐自习地点",
                "📚" to "整理 MIUIX 0.9.2 参考文档，docs 目录仅保留统一 reference"
            )
        ),
        "4.0" to VersionChangelog(
            items = listOf(
                "🧠" to "岱宗盒子 4.0：屁岱升级为可调用校园工具的 AI Agent，支持课表、成绩、考试、空教室、校园卡、通知、LMS、体测等多源查询",
                "🤝" to "接入学校交晓智服务，支持独立对话，并可作为屁岱的校内知识子代理参与多智能体协作",
                "📅" to "日程支持多学期切换，课表、考试、教材按所选学期查看；手动添加日程与教务课表并行合并，不会被刷新覆盖",
                "📱" to "移动交大支持内置访问与子服务登录接力，并补齐跑操等网页定位授权桥接",
                "🌐" to "WebVPN、内置浏览器、搜索与网页阅读链路继续加固，校内外访问更连贯",
                "⬆️" to "更新检查默认使用 Gitee，并可在设置中切换 GitHub；启动自动检查加入冷却，减少无意义请求",
                "🛡️" to "用户协议补充 AI、第三方模型服务、下载文件与校园系统请求频率说明",
                "🎨" to "完善 MIUIX 风格交互：会话管理、标题折叠、课表日期小字、体测与图书馆细节体验继续打磨"
            )
        ),
        "4.0-beta2" to VersionChangelog(
            items = listOf(
                "⚡" to "校园助手支持流式回复与随时停止生成，等待过程更自然",
                "🧠" to "适配 DeepSeek 思考模式与新模型，工具调用可连续保留推理上下文",
                "📅" to "校园助手新增校历与全校课程查询，可直接问假期、考试周、教师和开课地点",
                "☎️" to "新增原生校园黄页，支持机构分类、电话搜索和一键拨号，助手也能直接查询",
                "💠" to "新增原生交晓智：支持多会话、模型切换、流式回复与自动续期，并可作为屁岱的校内知识源",
                "🎛️" to "新增能力开关，可按需关闭联网、校园卡、成绩等工具",
                "🪧" to "课表、成绩、教室等富卡片会随会话保存，重进对话不再丢失",
                "🏠" to "首页问候去重，并将「问屁岱」加入常用功能候选"
            )
        ),
